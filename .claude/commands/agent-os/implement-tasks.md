...
PHASE 1: Determine which slice(s) from tasks.md should be implemented PHASE 2:
Delegate implementation to the implementer subagent PHASE 3: After ALL slices
have been implemented, delegate to implementation-verifier to produce the final
verification report.
...
### PHASE 1: Determine which slice(s) to implement
...
implementation of those specified slice(s) to the **implementer** subagent.
...
Delegate to the **implementer** subagent to implement the specified slice(s):

Provide to the subagent:

- How to create tasks: # Task List Creation
...
3. Implement the assigned slice according to requirements and standards
4. Update `agent-os/specs/[this-spec]/tasks.md` to mark completed tasks with
   `- [x]`

### PHASE 3: Produce the final verification report

IF ALL slices in tasks.md are marked complete with `- [x]`, then proceed with
this step. Otherwise, return to PHASE 1.
...
- The path to this spec: `agent-os/specs/[this-spec]` Instruct the subagent to
  do the following:
