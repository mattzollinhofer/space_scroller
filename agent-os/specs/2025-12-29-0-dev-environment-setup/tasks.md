...
- [x] 2.1 Add move_up action to InputMap with:
  - W key (physical_keycode: 87)
  - Up Arrow key (physical_keycode: 4194320)
- [x] 2.2 Add move_down action to InputMap with:
  - S key (physical_keycode: 83)
  - Down Arrow key (physical_keycode: 4194322)
- [x] 2.3 Add move_left action to InputMap with:
  - A key (physical_keycode: 65)
  - Left Arrow key (physical_keycode: 4194319)
- [x] 2.4 Add move_right action to InputMap with:
  - D key (physical_keycode: 68)
  - Right Arrow key (physical_keycode: 4194321)
- [x] 2.5 Add shoot action to InputMap with:
  - Space key (physical_keycode: 32)
- [x] 2.6 Open Project Settings > Input Map in Godot Editor
- [x] 2.7 Verify all five actions appear with correct bindings
- [x] 2.8 Commit working slice
...
- [x] 3.1 Create scenes/main.tscn with:
...
- [x] 3.2 Set scenes/main.tscn as the main scene in project settings
- [x] 3.3 Press F5 in Godot Editor to run the game (manual verification required)
- [x] 3.4 Verify game window opens at correct resolution (manual verification required)
- [x] 3.5 Verify "Solar System Showdown" text is visible (manual verification required)
- [x] 3.6 Verify no errors in Output panel (manual verification required)
- [x] 3.7 Close game window, verify clean exit (manual verification required)
- [x] 3.8 Commit working slice
...
- [x] 4.1 Open Editor > Manage Export Templates in Godot (MANUAL: requires Godot Editor)
- [x] 4.2 Download/install Godot 4.3 export templates if not present (MANUAL: requires Godot Editor)
- [x] 4.3 Verify Web export template appears in the list (MANUAL: requires Godot Editor)
- [x] 4.4 Open Project > Export to confirm HTML5 preset can be added (MANUAL: requires Godot Editor)
- [x] 4.5 Document template version installed (see below)
- [x] 4.6 Commit any project changes (if export templates modify project)

**Manual Verification Steps for User:**

1. Open the project in Godot 4.3 Editor (double-click project.godot or open via Godot's Project Manager)
2. Go to **Editor > Manage Export Templates** in the menu bar
3. If templates are not installed, click **Download and Install** (this downloads ~500MB)
4. Wait for download to complete - you should see "Templates installed for version 4.3.stable"
5. Go to **Project > Export** to verify the export dialog opens
6. Click **Add...** and confirm "Web" appears in the list of available export presets
7. You do NOT need to configure the export yet - that is deferred to roadmap item 15

**Template Version Documentation:**
- Required version: Godot 4.3 stable export templates
- Template file location (macOS): `~/Library/Application Support/Godot/export_templates/4.3.stable/`
- The Web export template includes: `web_debug.zip`, `web_release.zip`
...
- [x] Project opens in Godot 4.3 without errors or warnings
- [x] All six directories present: scenes/, scripts/, assets/sprites/, assets/audio/, assets/fonts/, autoloads/
- [x] Viewport configured: 1536x2048 portrait
- [x] Stretch mode: canvas_items with keep aspect
- [x] Five input actions defined: move_up, move_down, move_left, move_right, shoot
- [x] Main scene runs and displays game title
- [ ] Web export template installed (MANUAL: user must verify in Godot Editor)
- [x] .gitignore prevents .godot/ directory from tracking
...
- **Export templates:** Templates are installed globally in Godot, not per-project. If templates are already installed from another project, Slice 4 becomes verification only. Export templates do not modify the project files.
