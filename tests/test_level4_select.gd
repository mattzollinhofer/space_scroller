extends Node2D
## Integration test: Level 4 button appears in level select screen and can be clicked.
## Verifies Level 4 is accessible from the UI.
...
func _ready() -> void:
	print("=== Test: Level 4 Button in Level Select ===")
	...
	# Find Level 4 button
	var level4_button = _find_button_by_text(level_select, "Level 4")
	if not level4_button:
		level4_button = _find_node_by_name(level_select, "Level4Button") as Button

	if not level4_button:
		_fail("Level 4 button not found in level select")
		return

	print("Level 4 button found: %s" % level4_button.text)

	# Verify button is not disabled
	if level4_button.disabled:
		_fail("Level 4 button should not be disabled")
		return

	print("Level 4 button is enabled")
...
func _pass() -> void:
	_test_passed = true
	print("=== TEST PASSED ===")
	print("Level 4 button exists and is selectable in level select.")
