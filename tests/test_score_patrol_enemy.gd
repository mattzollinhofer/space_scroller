...
func _ready() -> void:
	print("=== Test: Patrol Enemy Awards 200 Points ===")
	...
	# Run the test
	await _run_patrol_enemy_test()


func _run_patrol_enemy_test() -> void:
	...
	if enemy.health != 2:
		_fail("Patrol enemy should have 2 HP, got %d" % enemy.health)
		return
	...
	print("Score after killing patrol enemy: %d" % new_score)

	var expected_score = initial_score + 200
	if new_score != expected_score:
		_fail("Expected score %d after killing patrol enemy, got %d" % [expected_score, new_score])
		return

	print("Patrol enemy awarded 200 points correctly!")
	_pass()
